    ...
    <div class="card shadow-sm soft-card machines-card">
        ...
        <?php foreach ($machines as $m): ?>
            <article class="machine-card">
                ...
                <?php if (!empty($m['_attachments'])): ?><div class="machine-file-chips"><?php foreach ($m['_attachments'] as $attachment): ?><?php $documentName = (string) ($attachment['original_name'] ?? 'Documento'); $documentMeta = gt_machine_attachment_document_meta($documentName); $isPdf = ($attachment['mime_type'] ?? '') === 'application/pdf' || strtolower(pathinfo($documentName, PATHINFO_EXTENSION)) === 'pdf'; ?><a class="machine-file-chip machine-file-<?= h($documentMeta['tone']) ?><?= $isPdf ? ' machine-pdf-preview' : '' ?>" href="<?= h(gt_machine_attachment_url($attachment)) ?>" target="_blank" rel="noopener" title="<?= h($documentName) ?>" aria-label="<?= h($documentMeta['label'] . ': ' . $documentName) ?>"<?= $isPdf ? ' data-pdf-name="' . h($documentName) . '"' : '' ?>><span class="machine-file-icon" aria-hidden="true"><i class="bi <?= h($documentMeta['icon']) ?>"></i></span><span class="machine-file-copy"><strong><?= h($documentMeta['label']) ?></strong><small><?= h($documentName) ?></small></span></a><?php endforeach; ?></div><?php endif; ?>
