...
try{
    ...
    check((float)$pdo->query('SELECT time_per_unit FROM erp_operations WHERE id='.$op)->fetchColumn()===0.0,'tempo de ciclo não fica guardado no catálogo de operações');
    ...
    $pdo->prepare('INSERT INTO erp_finished_products(code,description,status) VALUES (?,? ,"Ativo")')->execute([$tag.'ART','Artigo teste']);$article=(int)$pdo->lastInsertId();$version=$svc->createVersion($article,$uid);$step=$svc->addStep($version,$op,['primary_machine_id'=>$m1,'setup_time'=>10,'run_value'=>2,'calculation_unit'=>'minutes_per_unit','material_id'=>[$material],'material_quantity'=>[0.5],'material_reserve'=>[1]], $uid);
    check((float)$pdo->query('SELECT run_value FROM erp_article_routing_steps WHERE id='.$step)->fetchColumn()===2.0,'tempo de ciclo guardado na etapa do routing do artigo');
    ...
    $svc->updateStep($step,['operation_id'=>$op,'primary_machine_id'=>$m1,'operation_no'=>20,'operators_count'=>3,'setup_time'=>12,'run_value'=>4,'calculation_unit'=>'minutes_per_unit','base_quantity'=>1,'material_id'=>[$material],'material_quantity'=>[0.5],'material_reserve'=>[1]],$uid);
    check((float)$pdo->query('SELECT run_value FROM erp_article_routing_steps WHERE id='.$step)->fetchColumn()===4.0,'versão ativa editada sem criar uma nova versão');
    $svc->updateStep($step,['operation_id'=>$op,'primary_machine_id'=>'','operation_no'=>20,'operators_count'=>3,'setup_time'=>12,'run_value'=>4,'calculation_unit'=>'minutes_per_unit','base_quantity'=>1,'material_id'=>[$material],'material_quantity'=>[0.5],'material_reserve'=>[1]],$uid);
    check($pdo->query('SELECT primary_machine_id FROM erp_article_routing_steps WHERE id='.$step)->fetchColumn()===null&&(int)$pdo->query('SELECT COUNT(*) FROM erp_routing_step_machines WHERE routing_step_id='.$step)->fetchColumn()===0,'operação configurada sem máquina');
    $svc->deleteVersion($version2,$uid);check((int)$pdo->query('SELECT COUNT(*) FROM erp_article_routing_versions WHERE routing_id=(SELECT routing_id FROM erp_article_routing_versions WHERE id='.$version.')')->fetchColumn()===1,'versão excedente eliminada');
    try{$svc->deleteVersion($version,$uid);throw new RuntimeException('foi possível eliminar a última versão');}catch(DomainException $expected){}
    check((int)$pdo->query('SELECT COUNT(*) FROM erp_article_routing_versions WHERE id='.$version)->fetchColumn()===1,'última versão do routing protegida');
    ...
    check((float)$pdo->query('SELECT total_planned_minutes FROM erp_production_order_routing_snapshots WHERE production_order_id='.$order)->fetchColumn()===412.0,'cálculo de tempo previsto e snapshot da OF');
    ...
    $pdo->prepare('UPDATE erp_article_routing_steps SET run_value=99 WHERE routing_version_id=?')->execute([$version]);check((float)$pdo->query('SELECT run_value FROM erp_production_order_operations WHERE production_order_id='.$order)->fetchColumn()===4.0,'snapshot da OF permanece imutável');
