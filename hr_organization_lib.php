...
function gt_run_org_migrations(PDO $pdo) {
    ...
    $pdo->exec('CREATE TABLE IF NOT EXISTS erp_machine_suppliers (machine_id INTEGER NOT NULL, supplier_id INTEGER NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(machine_id, supplier_id), FOREIGN KEY(machine_id) REFERENCES erp_machines(id) ON DELETE CASCADE, FOREIGN KEY(supplier_id) REFERENCES erp_suppliers(id) ON DELETE RESTRICT)');
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_erp_machine_suppliers_supplier ON erp_machine_suppliers(supplier_id, machine_id)');
    $pdo->exec("INSERT OR IGNORE INTO erp_machine_suppliers(machine_id,supplier_id) SELECT m.id,s.id FROM erp_machines m JOIN erp_suppliers s ON LOWER(TRIM(s.name))=LOWER(TRIM(m.supplier)) WHERE TRIM(COALESCE(m.supplier,''))<>''");
