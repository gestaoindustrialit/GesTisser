...
class IntegrationClient
{
    ...
    public function request(array $integration, string $method='GET', string $endpoint='', $body=null, array $extra=[]): array
    {
        ...
        // Never expose credential values to the UI or logs.  Header names are
        // still useful when diagnosing what the connector actually sent.
        $requestHeaderNames=[];foreach($headers as $header){$separator=strpos($header,':');$requestHeaderNames[]=$separator===false?$header:substr($header,0,$separator);}
        ...
        $last['request_header_names']=array_values(array_unique($requestHeaderNames));
        return $last;
    }
}
