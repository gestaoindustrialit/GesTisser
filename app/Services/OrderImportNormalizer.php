...
{
    private const UNITS = ['UN'=>'UN','UND'=>'UN','PCS'=>'UN','PC'=>'UN','KG'=>'KG','KGS'=>'KG','M'=>'M','MT'=>'M','ROL'=>'ROL','ROLO'=>'ROL','PALETE'=>'PALETE'];
    ...
    public static function number(string $value): ?float
    {
        ...
        elseif ($comma !== false) $v = substr_count($v, ',') === 1 && strlen($v)-$comma-1 <= 2 ? str_replace(',', '.', $v) : str_replace(',', '', $v);
    ...
    {
        $value=trim($value); $formats=['!d/m/Y','!d-m-Y','!Y-m-d','!d M Y'];
