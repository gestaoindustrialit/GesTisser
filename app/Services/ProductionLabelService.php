...
final class ProductionLabelService
{
    ...
    {
        $stmt=$this->pdo->prepare('SELECT reference_key,validated_at FROM erp_production_labels WHERE production_order_id=? AND label_type=? ORDER BY COALESCE(updated_at,validated_at) DESC');
    ...
    public function save(int $orderId,string $type,string $reference,array $values,int $userId): array
    {
        ...
        else{$stmt=$this->pdo->prepare('INSERT INTO erp_production_labels(production_order_id,label_type,reference_key,values_json,validated_by,validated_at,updated_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)');$stmt->execute([$orderId,$type,$reference,$json,$userId]);$id=(int)$this->pdo->lastInsertId();}
        $this->pdo->prepare('INSERT INTO erp_production_label_history(production_label_id,values_json,validated_by,validated_at) VALUES (?,?,?,CURRENT_TIMESTAMP)')->execute([$id,$json,$userId]);
