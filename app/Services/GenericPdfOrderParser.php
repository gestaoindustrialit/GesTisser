...
final class GenericPdfOrderParser
{
    public function parse(string $text, array $context = []): array
    {
        ...
        if(preg_match('/(?:encomenda|order)\s*(?:n[.ºo°]*|number|no)?\s*[:#]?\s*([A-Z0-9.\/_-]+)/iu',$text,$m))$result['order_number']=$m[1];
        ...
        if(preg_match('/(?:entrega|delivery)\s*:?\s*(\d{1,2}[\/-]\d{1,2}[\/-]\d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}\s+[A-Za-z]{3}\s+\d{4})/iu',$text,$m))$result['delivery_date']=OrderImportNormalizer::date($m[1]);
        ...
        foreach($physical as $raw){$line=trim($raw);if($line===''||$this->isHeader($line))continue;
            ...
            $pattern='~^([A-Z0-9][A-Z0-9._/-]{1,})\s+(.+?)\s+(\d[\d .]*?(?:[,.]\d+)?)\s+(PALETE|ROLO|UND|PCS|KGS|ROL|UN|PC|KG|MT|M)(?:\s+(\d[\d .]*?(?:[,.]\d+)?)(?:\s+(\d[\d .]*?(?:[,.]\d+)?))?)?(?:\s+(\d{1,2}[/-]\d{1,2}[/-]\d{4}|\d{4}-\d{2}-\d{2}))?$~iu';
    ...
    private function flush(array &$result,?array &$pending):void{if($pending!==null){$pending['line_number']=count($result['lines'])+1;$pending['status']='confirmation';$result['lines'][]=$pending;$pending=null;}}
}
